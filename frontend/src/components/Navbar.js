    ...
    <nav className="fixed top-5 left-1/2 -translate-x-1/2 z-50 w-[92%] md:w-auto" data-testid="navbar">
      <div className="glass-navbar rounded-full px-4 md:px-6 h-14 flex items-center gap-2 md:gap-6">
        ...
        {user ? (
          ...
          <Button
            onClick={handleLogin}
            className="rounded-full bg-keen-black text-white hover:bg-keen-black/90 hover:scale-105 transition-all duration-300 text-sm px-5 h-9 flex items-center gap-2"
      ...
      {mobileOpen && (
        <div className="md:hidden glass-card rounded-3xl mt-2 p-4 space-y-1" data-testid="mobile-menu">
          ...
          {!user && (
            <button
              onClick={handleLogin}
              className="w-full mt-2 px-4 py-3 rounded-2xl bg-keen-black text-white text-sm font-medium flex items-center justify-center gap-2"
            >
